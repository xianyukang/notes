## Table of Contents
  ...
  - [Package](#Package)
    ...
    - [谨慎使用 init 函数](#%E8%B0%A8%E6%85%8E%E4%BD%BF%E7%94%A8-init-%E5%87%BD%E6%95%B0)
...
如何创建包?  一个文件夹就是一个包,  所以新建一个文件夹,  并让其中的 .go 文件使用同一个 `package` 名  
...
注意 `导入路径` 和 `包名` 是两个概念,  比如 `github.com/xxx/aaa` 这个导入路径下的包名可以为 `bbb`.  
As a general rule, you should make the name of the package match the name of the directory that contains the package. 比如 `math/rand` 是包的导入路径， 那么用 `rand` 作为包名.
...
![image-20220517133658034](https://static.xianyukang.com/img/image-20220517133658034.png) 
...
![image-20220517171752213](https://static.xianyukang.com/img/image-20220517171752213.png) 

### 下载依赖模块

严格来说我们下载的是一个 go module (模块), 一个模块中包含若干个 package

- 可以用 go get 下载模块: `go get github.com/go-sql-driver/mysql`  
- 可以用 @v1.5.1 后缀下载指定版本: `go get github.com/go-sql-driver/mysql@v1.5.1`  
- 存在 v2 版本的模块会在 module path 中以 /v2 结尾: `go get github.com/gdamore/tcell/v2`  
- 不写 v2 后缀时下载的是 v1 的最新版, 例如: `go get github.com/gdamore/tcell`
...
`go.sum` 文件记录了包的 hash,  用于验证所下载的包有没有被修改过,  确保大家下载了完全相同的包
...
We can see what versions of the module are available with the `go list` command:  
$ go.exe list -m -versions github.com/gdamore/tcell
...
go get -u=patch github.com/foo/bar
...
在 go.mod 中用 `// indirect` 注释哪些模块会被间接引用,  使用 go get 下载模块时会无脑加 // indirect 注释,  可以用 `go mod tidy` 清理项目依赖、标注哪些模块是间接依赖
...
![img](https://static.xianyukang.com/img/impver.png) 
