## Table of Contents
  - [LeetCode 热门 1000 题](#LeetCode-%E7%83%AD%E9%97%A8-1000-%E9%A2%98)
...
// 然后比较 2 和 5
// (1) 因为 2 < 5,  所以 2 不可能是第 k=4 大的元素,  因为就算 5 之前的数全都小于 2, 比 2 小的数最多也只有 2 个
// (2) 既然 2 不是第 4 大的元素,  2 前面的 1 就更不可能是,  所以排除掉 [1, 2]、共排除了 k/2 个元素

// (3) 若将两个数组合并成一个有序数组,  [1, 2] 必定位于第 4 大的元素的左侧
//     这时候把 [1, 2] 抽掉,  原本第 k=4 大的元素,  会变成 [3] 和 [4, 5, 6] 中第 k-2 = 2 大的元素
// (4) 整个问题转变成在 [3] 和 [4, 5, 6] 两个有序数组中查找第 k=2 大的元素
```
...
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
    ...
    mid1Value := getKth(nums1, nums2, mid1 + 1) // 索引加一就是第 k 大的数
